    ...
    protected boolean executeUpdate(String sql, Consumer<Throwable> exceptionHandler, Object... args) {
        try (Connection connection = getConnection();){
            ...
            for (int i = 0; i < args.length; i++) {
                Object arg = args[i];
                Class argType = arg.getClass();
                Class wrapperType = wrapperToPrimitive(arg.getClass());
                if (wrapperType == null){
                    wrapperType = argType;
                }
                Method method = PreparedStatement.class.getMethod(preparedStatementMethodMappings.get(wrapperType), int.class, wrapperType);
                method.invoke(preparedStatement, i+1, arg);
            }
