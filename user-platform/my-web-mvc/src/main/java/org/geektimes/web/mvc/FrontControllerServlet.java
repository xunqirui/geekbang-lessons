...
public class FrontControllerServlet extends HttpServlet {
    ...
    private void initHandleMethods() {
        for (Controller controller : ComponentContext.getInstance().getComponent(Controller.class)) {
            Class<?> controllerClass = controller.getClass();
            Path pathFromClass = controllerClass.getAnnotation(Path.class);
            String controllerPath = pathFromClass.value();
            ...
            for (Method method : publicMethods) {
                ...
                if (pathFromMethod != null) {
                    String requestPath = controllerPath;
                    String childPath = pathFromMethod.value();
                    if (!pathFromMethod.value().startsWith("/") && !"".equals(pathFromMethod.value())){
                        // 如果子路径不是 / 打头则加上 /
                        childPath = "/" + childPath;
                    }
                    requestPath += childPath;
                    handleMethodInfoMapping.put(requestPath,
                            new HandlerMethodInfo(requestPath, method, supportedHttpMethods));
                }

            }
            controllersMapping.put(controllerPath, controller);
        }
    }
    ...
    public void service(HttpServletRequest request, HttpServletResponse response)
        ...
        // 映射到 Controller
        Controller controller = getController(requestMappingPath);
...
//                         RequestDispatcher requestDispatcher = request.getRequestDispatcher(viewPath);
    ...
    private Controller getController(String requestPath){
        for (String key : controllersMapping.keySet()){
            if (requestPath.startsWith(key)){
                return controllersMapping.get(key);
            }
        }
        return null;
    }
